...
import PhotoPromptModal from '../PhotoPromptModal/PhotoPromptModal.js';
...
function ProductSearchModal({
  ...
  const [photoPrompt, setPhotoPrompt] = useState(null); // { product } or null
  ...
  const confirmAddQuantity = useCallback(() => {
    ...
    // Product has photos, add to cart normally
  ...
  const handlePhotoPromptComplete = useCallback((productWithPhotos) => {
    // Add the product to cart after photo prompt is handled
    const toAdd = qtyDialog ? Math.min(qtyDialog.quantity, qtyDialog.product.stock) : 1;
    const key = productWithPhotos.docId || productWithPhotos.id;
    
    onAddProduct(
      { ...productWithPhotos, docId: key, totalStock: productWithPhotos.originalStock },
      toAdd
    );

    showModalNotice(
      `Añadida${toAdd === 1 ? '' : 's'} ${toAdd} unidad${toAdd === 1 ? '' : 'es'} de "${productWithPhotos.name}".`,
      'success',
      2200
    );
  ...
  const handlePhotoUploaded = useCallback((updatedProduct, uploaded) => {
    // Show success message
    showModalNotice('Foto añadida correctamente.', 'success', 2000);
    
    // Add product to cart after photo upload
    handlePhotoPromptComplete(updatedProduct);
  }, [showModalNotice, handlePhotoPromptComplete]);
  ...
  return (
    ...
    >
      ...
      <PhotoPromptModal
        ...
        onPhotoSkipped={() => {
          if (photoPrompt) {
            handlePhotoPromptComplete(photoPrompt);
          }
        }}
        user={user}
      />
