...
function ProductSearchModalBuys({
          ...
          <div className="lps-qty-overlay" role="dialog" aria-modal="true" aria-label="Seleccionar cantidad">
            <div className="lps-qty-panel">
              ...
              <div className="lps-qty-control">
                <button type="button" className="lps-qty-btn" onClick={() => changeQuantity(-1)} disabled={qtyDialog.quantity <= 1}>−</button>
                ...
                <button type="button" className="lps-qty-btn" onClick={() => changeQuantity(1)}>+</button>
              </div>
  ...
  return (
    <div data-row-index={index} className={'lps-row' + (focused ? ' focused' : '')} role="option" aria-selected={focused} tabIndex={focused ? 0 : -1} onClick={onSelect} onKeyDown={handleKey}>
      <div className="lps-thumb">{product.image ? <img src={product.image} alt={product.name} loading="lazy" onError={(e) => { e.currentTarget.style.visibility = 'hidden'; }} /> : <span className="lps-thumb-ph" aria-hidden="true">?</span>}</div>
